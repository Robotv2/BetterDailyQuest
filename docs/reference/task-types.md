...
BDQ provides 20 task types. Choose the family that matches the player action.
...
| Player | `DEATH`, `LOCATION`, `WALK`, `SWIM` | [Player task reference](task-types-player.md) |
...
- `CARVE`, `MILK`, `SHEAR`, `WALK`, and `SWIM` need no target.

Most counted task types add one per event. `COOK`, `ENCHANT`, and `PICKUP` can use an item amount from the event. `WALK` and `SWIM` add distance in blocks. The detailed pages explain each rule.
